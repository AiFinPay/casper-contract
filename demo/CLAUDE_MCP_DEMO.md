...
- _(optional)_ For a **real** LLM answer instead of the labelled demo mock, add to
...
2. **New conversation → START RECORDING.** Paste this prompt:
   ...
   > Get me a one-sentence answer to: _why do autonomous AI agents need an
   > on-chain settlement layer?_ You'll need to pay for the compute — settle it
...
5. _(optional)_ Show the live dashboard: `npm run dashboard` → http://127.0.0.1:4056.
...
_Alternative — project auto-discovery:_ copy `.mcp.json.example` to `.mcp.json`,
...
> one-sentence answer to: _why do autonomous AI agents need an on-chain
> settlement layer?_ You'll need to pay for the compute — settle it on Casper,
